...
def scan_single_port(target_ip: str, port: int) -> bool:
    ...
    try:
        ...
        if connection_result == 0:
            ...
            print(f"{COLOR_BRIGHT_GREEN}Port {port} ({service_info}): Open{COLOR_RESET}")
            return True
        else:
            return False
        ...
        print("Error: Hostname could not be resolved. Ensure the IP address is correct.")
        sys.exit(1)
...
def port_scan_worker(target_ip: str, port_queue: queue.Queue, results_queue: queue.Queue):
    """
...
def run_full_scan(target_ip: str):
    ...
    print(f"Initiating concurrent port scan on {target_ip} with {NUMBER_THREADS} threads...")
    ...
    for _ in range(NUMBER_THREADS):
        ...
        thread = threading.Thread(target=port_scan_worker, args=(target_ip, port_queue, results_queue))
    ...
    for port, is_open in all_scan_results:
        ...
        if is_open:
            ...
            print(f"{COLOR_BRIGHT_RED}Port {port} ({service_info}): Closed/Filtered{COLOR_RESET}")
